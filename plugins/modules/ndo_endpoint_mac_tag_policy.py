...
options:
  ...
  uuid:
    ...
    type: str
    aliases: [ enpoint_mac_tag_policy_uuid ]
  bridge_domain:
    description:
...
EXAMPLES = r"""
- name: Create a Endpoint MAC Tag Policy
  cisco.mso.ndo_endpoint_mac_tag_policy:
    ...
    policy_tags:
      ...
      - key: key_2
        value: value_2
    state: present
  register: create_enpoint_mac_tag_policy

- name: Update a Endpoint MAC Tag Policy using MAC address and Bridge Domain
  cisco.mso.ndo_endpoint_mac_tag_policy:
    host: mso_host
    username: admin
    password: SomeSecretPassword
...
- name: Update a Endpoint MAC Tag Policy using UUID
  cisco.mso.ndo_endpoint_mac_tag_policy:
    host: mso_host
    username: admin
    password: SomeSecretPassword
    template: ansible_tenant_policy_template
    uuid: "{{ create_enpoint_mac_tag_policy.current.uuid }}"
...
- name: Query a Endpoint MAC Tag Policy using MAC address and Bridge Domain
  cisco.mso.ndo_endpoint_mac_tag_policy:
    host: mso_host
    username: admin
    password: SomeSecretPassword
...
- name: Query a Endpoint MAC Tag Policy using MAC address and VRF
  cisco.mso.ndo_endpoint_mac_tag_policy:
    host: mso_host
    username: admin
    password: SomeSecretPassword
...
- name: Query a Endpoint MAC Tag Policy using UUID
  cisco.mso.ndo_endpoint_mac_tag_policy:
    host: mso_host
    username: admin
    password: SomeSecretPassword
...
- name: Delete a Endpoint MAC Tag Policy using its MAC address and Bridge Domain
  cisco.mso.ndo_endpoint_mac_tag_policy:
    host: mso_host
    username: admin
    password: SomeSecretPassword
...
- name: Delete a Endpoint MAC Tag Policy using its MAC address and VRF
  cisco.mso.ndo_endpoint_mac_tag_policy:
    host: mso_host
    username: admin
    password: SomeSecretPassword
...
- name: Delete a Endpoint MAC Tag Policy using UUID
  cisco.mso.ndo_endpoint_mac_tag_policy:
    host: mso_host
    username: admin
    password: SomeSecretPassword
...
def main():
    ...
    argument_spec.update(
        ...
        uuid=dict(type="str", aliases=["enpoint_mac_tag_policy_uuid"]),
        bridge_domain=dict(
            type="dict",
            options=dict(
    ...
    if mac and not uuid and not (bd or vrf or vrf_uuid or bd_uuid):
        mso.fail_json(
            msg="When providing a MAC address without UUID, you need to provide one of the following attributes: 'bd', 'bd_uuid', 'vrf' or 'vrf_uuid'."
        )
    ...
    enpoint_mac_tag_policy_path = "/tenantPolicyTemplate/template/endpointMacTagPolicies/{0}".format(match.index if match else "-")
    ...
    if state == "present":
        ...
        if match:
            ...
            if bd_uuid and not vrf_uuid:
                remove_data.append("vrfRef")
            elif not bd_uuid and vrf_uuid:
                remove_data.append("bdRef")
            append_update_ops_data(ops, match.details, enpoint_mac_tag_policy_path, mso_values, remove_data)
            mso.sanitize(mso_values, collate=True)
        else:
            mso.sanitize(mso_values)
            ops.append(dict(op="add", path=enpoint_mac_tag_policy_path, value=mso.sent))

    elif state == "absent":
        if match:
            ops.append(dict(op="remove", path=enpoint_mac_tag_policy_path))
